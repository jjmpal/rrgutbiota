...
### Associations for hypertension looping covariates
...
Lactobacillus prevalence in subsample `r sprintf("%.1f%%", 100*pseq_prevalence(pseq.genus.salt))`. 
Levels of dU-Na `r sprintf("%.1f (%.1f)", mean(meta(pseq.genus)$dUNA, na.rm = TRUE), 
	sd(meta(pseq.genus)$dUNA, na.rm = TRUE))`
...
Prevalence for L. paracasei is
`r pseq_prevalence_format(pseq.species, "Lactobacillus_paracasei (Bacteria)")`.
and for L. salivarius is 
`r pseq_prevalence_format(pseq.species, "Lactobacillus_salivarius (Bacteria)")`. 
...
Urinary sodium quartiles for L. paracei

```{r species salt levels paracasei, echo = FALSE}
df.species.salt %>%
    group_by(Lactobacillus_paracasei.Bacteria_group) %>%
    summarize(mean = round(mean(dUNA), 1),
              sd = round(sd(dUNA), 1)) %>%
    kable
```

Urinary sodium quartiles for L. salivarius

```{r salt levels salivarius, echo = FALSE}
df.species.salt %>%
    group_by(Lactobacillus_salivarius.Bacteria_group) %>%
    summarize(mean = round(mean(dUNA), 1),
              sd = round(sd(dUNA), 1)) %>%
    kable
```
